...
nni_sock_open(nni_sock **sockp, const nni_proto *proto)
{
	...
	if (((rv = nni_init()) != 0) ||
		...
		printf("vals are %d %d\n", nni_init(),
		    nni_sock_create(&s, proto));
		printf("rv returning with %d\n", rv);
		return (rv);
	}
	printf("nni_mtx_lock\n");
	nni_mtx_lock(&sock_lk);
    ...
    nni_sock *s, const char *name, const void *v, size_t sz, nni_type t)
{
	...
	if (rv != NNG_ENOTSUP) {
		nni_mtx_unlock(&s->s_mx);
		printf("nni_mtx_unlock(&s->s_mx); %d\n", rv);
		return (rv);
	}
	...
	if (rv != NNG_ENOTSUP) {
		nni_mtx_unlock(&s->s_mx);
		printf("nni_mtx_unlock(&s->s_mx)2; %d\n", rv);
		return (rv);
	}
	nni_mtx_unlock(&s->s_mx);
	...
	NNI_LIST_FOREACH (&s->s_listeners, l) {
		...
		if (x != NNG_ENOTSUP) {
			if ((rv = x) != 0) {
				nni_mtx_unlock(&s->s_mx);
				nni_free_opt(optv);
				printf("nni_listener_setopt %d\n",
				    nni_listener_setopt(
				        l, optv->name, optv->data, sz, t));
				return (rv);
			}
		}
	}
